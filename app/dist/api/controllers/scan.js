...
async function destroy(req, res, next) {
    try {
        const { id } = req.params;
        const { count, rows } = await AppointmentScan.findAndCountAll({
            where: { scanId: id },
        });
        if (count > 0) {
            next({
                status: 500,
                error: "Can not delete this scan, because it is associated with some appointments.",
            });
        }
        else {
