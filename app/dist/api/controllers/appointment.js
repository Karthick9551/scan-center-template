...
const { Appointment, AppointmentScan, Scan, Report } = db;
...
async function create(req, res, next) {
    try {
        const { phone, patientName, patientAge, patientGender, referringDoctorName, date, time, advanceAmount, totalAmount, scanId, isAdvancePaid, isTotalAmountPaid, isByDoctor, } = req.body;
        ...
        const appointment = await Appointment.create({
            ...
            totalAmount,
            isActive: true,
            token: isByDoctor ? lastToken + 1 : undefined,
            status: isByDoctor ? 1 : 0,
            isAdvancePaid: isAdvancePaid ? true : false,
            isTotalAmountPaid: isTotalAmountPaid ? true : false,
        }
        // { transaction: t }
        );
...
async function update(req, res, next) {
    try {
        const { id, phone, email, patientName, patientAge, patientGender, referringDoctorName, date, time, scanId, totalAmount, isTotalAmountPaid, } = req.body;
    ...
    try {
        ...
        upload(req, res, async function (err) {
            if (err) {
                ...
                if (appointment && appointment.id) {
                    // add reports
                    const affectedReportRow = await Report.create({
                        appointmentId: id,
                        name: name ? name.toString() : "",
                        fileName: fileName ? fileName.toString() : "",
                    });
                    if (!affectedReportRow || !affectedReportRow.id) {
                        return res.status(400).json({
                            error: "Failed to upload a file. Please try again",
                        });
                    }
...
async function destroyReport(req, res, next) {
    try {
        const { id } = req.params;
        const affectedRows = await Report.destroy({ where: { id } });
        if (affectedRows) {
            return res.status(200).json({
            ...
            return res.status(400).json({
                error: `Report not deleted. Please try again`,
            });
        }
    }
    catch (error) {
        console.log("\n\nError deleting Report ", error, "\n\n");
        next({ status: 500, error: "Db error deleting Report" });
    }
}
async function readPatientsByPhone(req, res, next) {
    try {
        const { phone } = req.params;
        const { filter, pagination } = res.locals;
        filter.where = { ...filter.where, phone };
        const { count, rows } = await Appointment.findAndCountAll({
            attributes: ["patientName", "patientAge", "patientGender"],
            ...filter,
            distinct: true,
        });
        if (rows && rows.length > 0) {
...
export { appointments, create, read, readByPhone, update, destroy, updateStatus, updatePayment, uploadReport, destroyReport, readPatientsByPhone, };
